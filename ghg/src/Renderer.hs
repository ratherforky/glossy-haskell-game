...
game2Pic (Play {for = f, opacity = b, fall = p, word = w}) = Translate (-xLength/2) (-yLength/2) (Pictures (bg ++ rendBack b ++ rendFor f yLength ++ rendFall p yLength ++ (fmap (Translate (xLength) (0)) (rendWord w)))) where
  bArr    = unOpacity b
...
rendBack :: Opacity -> [Picture]
rendBack (Opacity ps) = arrayBasic (fmap (fmap f) ps) where
  f :: Int -> Picture
  f x = Color (makeColor 0 0 0 (1 - g x)) (basicBlock blockSize blockSize)
  g :: Int -> Float
