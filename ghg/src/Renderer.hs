...
game2Pic (Play {for = f, opacity = b, fall = p, word = w, foundChars = cs}) = Translate (-xLength/2) (-yLength/2) (Pictures (bg ++ rendBack b yLength ++ rendFor f yLength ++ rendFall p yLength ++ (fmap (Translate (xLength) (0)) (rendWord w)) ++ (fmap (Translate (xLength) (yLength/2)) (rendWord cs)))) where
  --bArr    = unOpacity b
